...
import { Search, X, ArrowRight, AlertCircle, RefreshCw, Award, Calendar, GraduationCap, ChevronLeft, ChevronRight, Briefcase } from 'lucide-react'
...
export default function ManagementCollegesPage() {
  ...
  // Filter colleges for management category
  const managementColleges = useMemo(() => {
    return collegesResponse.colleges.filter((college: any) => 
      college.categories && college.categories.includes('management')
    )
  }, [collegesResponse.colleges])

  const colleges = managementColleges
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-slate-600 mb-6">{error?.message || 'Failed to load management colleges'}</p>
          <Button 
            onClick={() => refetch()}
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      {/* Hero Header */}
      <div className="relative bg-gradient-to-br from-[#8B5CF6] via-[#8B5CF6]/90 to-slate-900 text-white">
        <BackgroundSlider>
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-8 pb-16">
        {colleges.length === 0 ? (
          <div className="text-center py-16">
            ...
            <h3 className="text-2xl font-bold text-slate-900 mb-2">No Management Colleges Found</h3>
            <p className="text-slate-600 mb-6">
              Try adjusting your search terms or filters to find management colleges.
            </p>
            <Button onClick={() => refetch()} className="bg-[#8B5CF6] hover:bg-[#8B5CF6]/90">
              <RefreshCw className="w-4 h-4 mr-2" />
              Refresh Results
            </Button>
          </div>
          ...
          <>
            ...
            <div className="flex justify-between items-center mb-8">
              <div>
                <h2 className="text-2xl font-bold text-slate-900">
                  {colleges.length} Management Colleges Found
                </h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-12">
              {paginatedColleges.map((college: any) => (
                <div
                  key={college._id}
                  className="group bg-white rounded-2xl shadow-lg hover:shadow-2xl hover:shadow-[#8B5CF6]/10 transition-all duration-500 overflow-hidden"
                >
                  {/* College Banner */}
                  <div className="relative h-48 bg-gradient-to-br from-[#8B5CF6] to-[#8B5CF6]/80 overflow-hidden">
                    {college.banner_url ? (
                      <img
                  ...
                  <div className="p-6">
                    <div className="mb-4">
                      <h3 className="text-xl font-bold text-slate-900 mb-2 group-hover:text-[#8B5CF6] transition-colors">
                        {college.name}
                      </h3>
                      <div className="flex items-center gap-2 text-sm text-slate-600 mb-3">
                        ...
                        {college.city && (
                          <>
                            <span>•</span>
                            <span className="text-[#8B5CF6] font-medium">{college.city}</span>
                          </>
                        )}
                      </div>
                    </div>
                    ...
                    <Link href={`/colleges/${college.slug}`}>
                      <button className="w-full bg-[#8B5CF6] hover:bg-[#8B5CF6]/90 text-white font-bold py-3 px-4 rounded-xl text-sm flex items-center justify-center gap-2 transition-all duration-300 shadow-lg">
                        View Details
            ...
            {totalPages > 1 && (
              <div className="flex items-center justify-between">
                <div className="text-sm text-slate-600">
                  Showing {((currentPage - 1) * itemsPerPage) + 1} to {Math.min(currentPage * itemsPerPage, colleges.length)} of {colleges.length} management colleges
                </div>
