...
import { Search, X, ArrowRight, AlertCircle, RefreshCw, Award, Calendar, GraduationCap, ChevronLeft, ChevronRight, BookOpen } from 'lucide-react'
...
export default function EngineeringCollegesPage() {
  ...
  // Filter colleges for engineering category
  const engineeringColleges = useMemo(() => {
    return collegesResponse.colleges.filter((college: any) => 
      college.categories && college.categories.includes('engineering')
    )
  }, [collegesResponse.colleges])

  const colleges = engineeringColleges
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-slate-600 mb-6">{error?.message || 'Failed to load engineering colleges'}</p>
          <Button 
            onClick={() => refetch()}
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      {/* Hero Header */}
      <div className="relative bg-gradient-to-br from-[#1A4AB2] via-[#1A4AB2]/90 to-slate-900 text-white">
        <BackgroundSlider>
          ...
          <div className="text-center mb-16">
            ...
            <h1 className="text-4xl md:text-6xl font-extrabold tracking-tight mb-6">
              Top <span className="text-[#FACC15]">Engineering</span> Institutions
            </h1>
            <p className="text-xl text-white/90 max-w-3xl mx-auto leading-relaxed mb-8">
              Discover premier engineering colleges offering B.Tech, M.Tech programs with excellent placement records, 
            ...
            <div className="flex flex-wrap justify-center gap-8 text-center">
              <div>
                ...
                <div className="text-sm text-white/70 uppercase tracking-wider">Engineering Colleges</div>
              </div>
              <div>
                <div className="text-3xl font-bold text-[#FACC15]">IITs & NITs</div>
                <div className="text-sm text-white/70 uppercase tracking-wider">Premier Institutes</div>
              </div>
              <div>
                <div className="text-3xl font-bold text-[#FACC15]">JEE Main</div>
          ...
          <div className="max-w-2xl mx-auto px-4 sm:px-6 w-full">
            <div className="bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl p-2">
              <div className="relative">
                ...
                <Input
                  type="text"
                  placeholder="Search engineering colleges by name, location, or specialization..."
                  value={searchTerm}
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-8 pb-16">
        {colleges.length === 0 ? (
          <div className="text-center py-16">
            ...
            <h3 className="text-2xl font-bold text-slate-900 mb-2">No Engineering Colleges Found</h3>
            <p className="text-slate-600 mb-6">
              Try adjusting your search terms or filters to find engineering colleges.
            </p>
            ...
            <div className="flex justify-between items-center mb-8">
              <div>
                <h2 className="text-2xl font-bold text-slate-900">
                  {colleges.length} Engineering Colleges Found
                </h2>
            ...
            {totalPages > 1 && (
              <div className="flex items-center justify-between">
                <div className="text-sm text-slate-600">
                  Showing {((currentPage - 1) * itemsPerPage) + 1} to {Math.min(currentPage * itemsPerPage, colleges.length)} of {colleges.length} engineering colleges
                </div>
